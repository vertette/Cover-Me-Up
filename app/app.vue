...
const cmsResFullArray = {
  ...
  steam: [
    ...
    { name: 'Library header *', value: '920x430_2' },
    { name: 'Library hero *', value: '3840x1240' },
    { name: 'Event cover *', value: '800x450' },
    { name: 'Event header', value: '1920x622' },
    { name: 'Shortcut icon *', value: '512x512' },
    { name: 'App icon *', value: '184x184' },
  ],
  gog: [
...
const setImage = async (index) => {
  ...
  if (currentLayer.bgImage !== false) setImgOverlayStyle(false, getCurrentImage().getBoundingClientRect())
  else setImgOverlayStyle()
}
...
const addLayer = () => {
  ...
  const layerNameRegex = /^Layer #(\d+)$/im
...
const imgOverlayElem = useTemplateRef('imgOverlayElem'),
  isDraggingImage = ref(false),
  imgOverlayStyle = ref(false)
let dragStartClientX = 0,
  ...
  dragUnitX = 'px',
  dragUnitY = 'px'
const setImgOverlayStyle = (event, bound = false) => {
  set(imgOverlayStyle, bound)
}
const calculateDragElemStyle = (event) => {
  setImgOverlayStyle(false, event.target.getBoundingClientRect())
}
const onPointerDown = (event) => {
  if (!get(currentLayer) || get(currentLayer).locked) return
  setImgOverlayStyle(false, useElementBounding(getCurrentImage()))
  ...
  const parsedX = parseValue(get(currentLayer).bgImageHorPos || '50%') || { num: 50, unit: '%' }
  const parsedY = parseValue(get(currentLayer).bgImageVerPos || '50%') || { num: 50, unit: '%' }

  dragStartNumX = parsedX.num
  dragStartNumY = parsedY.num
  dragUnitX = parsedX.unit || 'px'
  dragUnitY = parsedY.unit || 'px'

  // lock dragging state
  set(isDraggingImage, true)
  ...
  // attach global listeners so we keep running code even if cursor leaves the element
...
const onPointerMoveImage = (event) => {
  ...
  // zoomScale is percentage (e.g. 100 == 1:1). Movement in screen px needs to be converted
  const scaleFactor = 100 / Math.max(1, get(zoomScale)) // multiply screen delta by this to get original px delta

  const deltaScreenX = event.clientX - dragStartClientX
  const deltaScreenY = event.clientY - dragStartClientY
  ...
  if (get(currentLayer).bgVerFlip) deltaInUnitY = -deltaInUnitY

  // compute new numeric values
...
const stopImageDrag = () => {
  ...
  set(isDraggingImage, false)
  window.removeEventListener('pointermove', onPointerMove)
  window.removeEventListener('pointerup', onPointerUp)
  window.removeEventListener('pointercancel', onPointerUp)
}
const handleClickOutsideImg = (event) => {
  if (!get(imgOverlayStyle)) return
  if (get(imgOverlayElem)?.contains(event.target) || getCurrentImage().contains(event.target)) return
...
onMounted(() => {
  document.addEventListener('mousedown', handleClickOutsideImg)
})
onUnmounted(() => {
  document.addEventListener('mousedown', handleClickOutsideImg)
})
...
<template>
  ...
  <div class="relative flex-1 overflow-hidden" ref="layerWrapperElem" @wheel="setZoomScale(0, $event)">
    <div class="absolute top-1/2 left-1/2 -translate-1/2 overflow-hidden border border-slate-300">
      ...
      <div v-show="gridSettings.visible && !inPreview && !isExporting" class="pointer-events-none absolute inset-0 top-0 left-0 z-50">
        <div
  ...
  <div
    ...
    :style="`top: ${imgOverlayStyle.top}px; left: ${imgOverlayStyle.left}px; width: ${imgOverlayStyle.width}px; height: ${imgOverlayStyle.height}px`"
  ...
  <div class="pointer-events-none absolute right-6 bottom-5 left-6 flex items-end justify-between gap-2 *:pointer-events-auto">
    <window class="w-88 2xl:w-96" :class="{ 'pointer-events-none opacity-0': inPreview === true }">
      ...
      <div class="flex items-center gap-x-1">
        <label for="width"><a href="https://css-tricks.com/almanac/properties/w/width/" target="_blank">Background width</a></label>
        <button class="transparent smallest invisible h-0"></button>
        <label for="height"><a href="https://css-tricks.com/almanac/properties/h/height/" target="_blank">Background height</a></label>
      </div>
      <div class="flex items-center gap-x-1">
        <div class="flex w-full">
          <button
